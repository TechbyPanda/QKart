...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    switch(true){
      case total > balance:
        errorMessage = "You do not have enough balance in your wallet for this purchase";
        break;
      case !addresses.all.length:
        errorMessage = "Please add a new address before proceeding.";
        break;
      case !addresses.selected:
        errorMessage = "Please select one shipping address to proceed.";
        break;
      default:
        return true;
    }
    enqueueSnackbar(errorMessage, {variant: 'alert'});
    return false;
  };
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               {addresses.all.length === 0 ? (
                ...
                <Grid container spacing={2} direction="row" my="1rem">
                  {addresses.all.map((data, index) => (
                    <Grid key={data._id} item xs={12}>
                      <Paper 
                        variant="outlined"
                        // className={`address-item ${addresses.selected === data._id && "selected"}`}
                        className={`${addresses.selected === data._id ? "address-item selected" : "address-item"}`}
                        onClick={() => {
                          setAddresses((prev) => ({
                            ...prev,
                            selected: data._id,
                      ...
                      >
                        ...
                        <Button variant="text" startIcon={<DeleteIcon />} onClick={() => deleteAddress(token, data._id)}>
                          Delete
                        </Button>
                      </Paper>
                    </Grid>
            ...
            <Button
              ...
              variant="contained"
              onClick={() => {
                validateRequest(items, addresses) && performCheckout(token, items, addresses);
              }}
            >
              PLACE ORDER
            </Button>
